...
\subsection{$K=30$ vs $K=60$ Comparison}
To strengthen the cluster-count decision, the project also compares $k=30$ and $k=60$ directly using silhouette, cluster-size structure, and representative topic examples. This comparison matters because a lower $k$ can still appear visually reasonable while merging distinct subtopics into broader groups. The empirical result shows that $k=60$ achieves the stronger silhouette score and produces a more fine-grained topical partition. The representative topic labels further show that AI, networking, optimization, and software-oriented themes separate more clearly at $k=60$ than at $k=30$.
...
\caption{Two-dimensional projection of the selected 60-cluster topic structure.}
\label{fig:clusters}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{../outputs/figures/k30_vs_k60_comparison.png}
\caption{Direct comparison of $k=30$ and $k=60$ using silhouette, cluster-size structure, and representative topics.}
\label{fig:kcompare}
\end{figure}
